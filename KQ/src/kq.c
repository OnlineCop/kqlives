...
void readcontrols(void)
{
   ...
   if (key[kdown])  down=1;
   if (key[kleft])  left=1;
   if (key[kright]) right=1;

   if (key[kesc])   besc=1;
   if (key[kalt])   balt=1;
   if (key[kctrl])  bctrl=1;
   if (key[kenter]) benter=1;

   if (key[KEY_ALT] && key[KEY_X]) program_death("X-ALT pressed... exiting.");

   if (key[KEY_F12]) data_dump();
...
void data_dump(void)
{
   ...
   if (!ff) program_death("could not open treasure.log!");
   for (a = 0 ; a < 200 ; a++) fprintf(ff,"%d = %d\n",a,treasure[a]);
   fclose(ff);
...
void change_map(char *map_name, int msx, int msy, int mvx, int mvy)
{
   ...
   for (o = 0 ; o < MAX_ANIM ; o++) adelay[o] = 0;
   ...
   for (i = 0 ; i < MAX_TILES ; i++) tilex[i] = i;
   for (i = 0 ; i < MAX_ANIM  ; i++) adata[i] = tanim[g_map.tileset][i];
...
void deallocate_stuff(void)
{
   ...
   if (map_seg)  free(map_seg);
   if (b_seg)    free(b_seg);
   if (f_seg)    free(f_seg);
   if (z_seg)    free(z_seg);
   if (s_seg)    free(s_seg);
   if (o_seg)    free(o_seg);
   if (progress) free(progress);
   if (treasure) free(treasure);
