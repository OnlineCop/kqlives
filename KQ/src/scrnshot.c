...
/*	save_screenshot()
	...
	Parameters:
		...
		[in] prefix: the first 4 to 7 characters of the name
		...
		if (prefix_len < SAVE_NAME_SIZE) {
			...
			*/
			if (!al_findfirst(filename, &old_file, 255)) {
				do {
					...
					if (strlen(get_filename(old_file.name)) == SAVE_NAME_SIZE+4) {
						...
						strncpy(buffer, get_filename(old_file.name) + prefix_len, SAVE_NAME_SIZE - prefix_len);
						buffer[SAVE_NAME_SIZE-prefix_len] = 0;

						/* finally, if the the screenshot number is larger, remember it */
						if (atoi(buffer) > last) last = atoi(buffer);
					}
			...
			if (++last < (int)pow(10,SAVE_NAME_SIZE-prefix_len)) {
				...
				temp = (src_bmp == screen) ? create_sub_bitmap(screen, 0,0, SCREEN_W, SCREEN_H) : src_bmp;
				if (temp) {
					get_palette(pal);
					save_bitmap(filename, temp, pal);
					if (src_bmp == screen) destroy_bitmap(temp);
				}
				else {
					TRACE("the bitmap [0x%p] is invalid (or unable to create sub bitmap)\n", src_bmp);
