  ...
  for (a=PSIZE;a<PSIZE+numens;a++)
  {
    if (fighter[a].sts[S_DEAD] == 0)
    {
      ...
      else
      {
        ...
        {
          if ((csts == S_BLESS && fighter[a].sts[csts] < 3) || (csts == S_STRENGTH && fighter[a].sts[csts] < 2) || (csts != S_BLESS && csts != S_STRENGTH && fighter[a].sts[csts] == 0))
          {
            tmpd[ne] = a;
            ne++;
          }
...
*/
int can_attack(int tgt)
{
  if (fighter[tgt].mhp < 1 || fighter[tgt].hp < 1 || fighter[tgt].sts[S_DEAD] != 0)
    return 0;
  return 1;
}
