...
*/
void draw_itemmenu(int ptr, int pg, int sl)
{
  int z,j,k,a,ck;
  ...
  for (k=0;k<16;k++)
  {
    ...
    if (j == I_SSTONE && use_sstone == 0)
      ck = FDARK;
    print_font(double_buffer,96+xofs,k*8+68+yofs,items[j].name,ck);
    if (z > 1)
    {
      sprintf(strbuf,"^%d",z);
  ...
  if (sl == 0)
  {
    ...
    print_font(double_buffer,160-a+xofs,212+yofs,items[g_inv[pg*16+ptr][0]].desc,FNORMAL);
...
int confirm_drop(void)
{
  ...
  print_font(double_buffer,104+xofs,212+yofs,"Confirm/Cancel",FNORMAL);
  blit2screen(xofs,yofs);
  while (!stop)
  {
    readcontrols();
...
*/
void camp_item_targetting(int pp)
{
  int t1,tg,z;
...
int item_effects(int sa, int t, int ti)
{
  ...
  switch(ti)
  {
    case I_MHERB: case I_SALVE: case I_PCURING:
    ...
    case I_OSEED: case I_EDROPS:
      ...
      break;
    case I_NLEAF: case I_NPOULTICE: case I_KBREW:
    ...
    case I_RRUNE:
      ...
      break;
    case I_ERUNE: case I_FRUNE: case I_WRUNE: case I_IRUNE:
      if (in_combat == 0)
        return 0;
      tmp = items[ti].elem;
      for (i=se;i<se+sen;i++)
  ...
  if (ti >= I_STRSEED && ti <= I_WISSEED)
  {
    ...
    switch(z)
    {
      case 0: message("Strength up!",255,0,xofs,yofs); break;
      case 1: message("Agility up!",255,0,xofs,yofs); break;
      case 2: message("Vitality up!",255,0,xofs,yofs); break;
      case 3: message("Intellect up!",255,0,xofs,yofs); break;
      case 4: message("Wisdom up!",255,0,xofs,yofs); break;
    }
    return 2;
  }
  if ((items[ti].icon == W_SBOOK || items[ti].icon == W_ABOOK))
  {
    ...
    tmp = 0;
    for (i=0;i<60;i++)
      if (party[pidx[t]].spells[i] == items[ti].hnds || party[pidx[t]].lvl < items[ti].ilvl)
        tmp = 1;
    if (tmp == 1)
      return 0;
    tmp = items[ti].hnds;
