...
void revert_cframes(int who,int aflag)
{
  ...
  {
    for (p=0;p<MAXCFRAMES;p++)
      blit(tcframes[who][p],cframes[who][p],0,0,0,0,fighter[who].cw,fighter[who].cl);
  }
}
...
void drawchar(int xw, int yw)
{
  ...
  for (i=0;i<PSIZE+noe;i++)
  {
    ...
    if (i < PSIZE && i < numchrs)
    {
      ...
      if (g_map.tileset == 0)
      {
        if (is_forestsquare(g_ent[i].tilex,g_ent[i].tiley))
        {
          ...
          if (g_ent[i].moving == 0)
            f = 1;
          if (g_ent[i].moving == 1 && is_forestsquare(g_ent[i].tilex,g_ent[i].tiley-1))
            f = 1;
          if (g_ent[i].moving == 2 && is_forestsquare(g_ent[i].tilex,g_ent[i].tiley+1))
            f = 1;
          if (g_ent[i].moving == 3 && is_forestsquare(g_ent[i].tilex+1,g_ent[i].tiley))
            f = 1;
          if (g_ent[i].moving == 4 && is_forestsquare(g_ent[i].tilex-1,g_ent[i].tiley))
            f = 1;
          if (f == 1)
          {
            clear_to_color(tc,0);
    ...
    else
    {
      if (g_ent[i].active)
      {
        if (g_ent[i].tilex >= view_x1 && g_ent[i].tilex <= view_x2 && g_ent[i].tiley >= view_y1 && g_ent[i].tiley <= view_y2)
        {
          if (dx >= -16 && dx <= 336 && dy >= -16 && dy <= 256)
          {
            ...
            {
              if (g_ent[i].transl == 0)
                draw_sprite(double_buffer,eframes[g_ent[i].chrx][fr],dx,dy);
              else
                draw_trans_sprite(double_buffer,eframes[g_ent[i].chrx][fr],dx,dy);
            }
            else
            {
              if (g_ent[i].transl == 0)
                draw_sprite(double_buffer,frames[g_ent[i].eid][fr],dx,dy);
              else
                draw_trans_sprite(double_buffer,frames[g_ent[i].eid][fr],dx,dy);
            }
          }
        }
      }
...
int is_forestsquare(int fx, int fy)
{
  ...
  if (f == 63 || f == 65 || f == 66 || f == 67 || f == 71 || f == 72 || f == 73 || f == 74)
    return 1;
  else
    return 0;
}
  ...
  for (dy=0;dy<16;dy++)
  {
    for (dx=0;dx<21;dx++)
    {
      if (ytc+dy >= view_y1 && xtc+dx >= view_x1 && ytc+dy <= view_y2 && xtc+dx <= view_x2)
      {
        ...
        blit(map_icons[0],double_buffer,0,0,dx*16+xofs,dy*16+yofs,16,16);
    }
  }
}
...
*/
void border(BITMAP *where,int x, int y, int x2, int y2)
{
...
void draw_textbox(int bstyle)
{
  ...
  if (bstyle == B_TEXT)
  {
    ...
    border(double_buffer,gbbx+xofs,gbby+yofs,gbbx+xofs+wid-1,gbby+yofs+hgt-1);
    destroy_bitmap(tm);
    if (gbt != -1)
  ...
  else
  {
    ...
    draw_sprite(double_buffer,bord[2],gbbx+xofs+wid-8,gbby+yofs);
    draw_sprite(double_buffer,bord[5],gbbx+xofs,gbby+yofs+hgt-8);
    draw_sprite(double_buffer,bord[7],gbbx+xofs+wid-8,gbby+yofs+hgt-8);
    for (a=0;a<gbbw;a++)
    {
      draw_sprite(double_buffer,bord[1],a*8+gbbx+xofs+8,gbby+yofs);
      draw_sprite(double_buffer,bord[6],a*8+gbbx+xofs+8,gbby+yofs+hgt-8);
    }
    for (a=0;a<gbbh;a++)
    {
      draw_sprite(double_buffer,bord[3],gbbx+xofs,a*12+gbby+yofs+8);
      draw_sprite(double_buffer,bord[4],gbbx+xofs+wid-8,a*12+gbby+yofs+8);
    }
    destroy_bitmap(tm);
    if (gbt != -1)
...
*/
void generic_text(int who, int box_style)
{
  int a,stop=0;
...
*/
char *parse_string(char *the_string)
{
  int a,who=-1,flag=0;
  char crud[255];
  int z;
...
*/
int prompt(int who, int numopt, int bstyle, char *sp1, char *sp2, char *sp3, char *sp4)
{
...
void message(char *m, int icn, int delay, int x_m, int y_m)
{
  ...
  if (icn == 255)
  {
    menubox(double_buffer,152-(strlen(msg)*4)+x_m,108+y_m,strlen(msg),1,DARKBLUE);
    print_font(double_buffer,160-(strlen(msg)*4)+x_m,116+y_m,msg,FNORMAL);
  }
  else
  {
    menubox(double_buffer,148-(strlen(msg)*4)+x_m,108+y_m,strlen(msg)+1,1,DARKBLUE);
    draw_icon(double_buffer,icn,156-(strlen(msg)*4)+x_m,116+y_m);
    print_font(double_buffer,164-(strlen(msg)*4)+x_m,116+y_m,msg,FNORMAL);
  }
  blit2screen(x_m,y_m);
  if (delay == 0)
    wait_enter();
