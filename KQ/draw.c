...
*/
void
draw_stsicon (BITMAP * where, int cc, int who, int inum, int icx, int icy)
{
   int j, st = 0, s;
...
/*
   Draw the zone layer.  Accounts for parallaxing.
*/
void draw_zonelayer (void)
{
   int dx, dy, pix, xtc, ytc;
	...
	for (dx = 0; dx < 21; dx++)
	  {
		 ...
		 && ytc + dy <= view_y2 && xtc + dx <= view_x2)
	       {
		  pix = z_seg[((ytc + dy) * g_map.xsize) + xtc + dx];
		  if (pix > 0)
		     rect (double_buffer, dx * 16 + xofs, dy * 16 + yofs,
			   dx * 16 + xofs + 15, dy * 16 + yofs + 15, pix);
	       }
	  }
     }
}
...
void draw_textbox (int bstyle)
{
   ...
   if (bstyle == B_TEXT)
	...
	draw_trans_sprite (double_buffer, tm, gbbx + 2 + xofs,
			   gbby + 2 + yofs);
...
*/
int
prompt (int who, int numopt, int bstyle, char *sp1, char *sp2, char *sp3,
	char *sp4)
{
