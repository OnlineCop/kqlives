...
int combat(int bno)
{
  ...
  {
    if (stretch_view == 1)
      stretch_blit(double_buffer,screen,a*16+xofs,a*12+yofs,320-(a*32),240-(a*24),0,0,640,480);
    else
      stretch_blit(double_buffer,screen,a*16+xofs,a*12+yofs,320-(a*32),240-(a*24),0,0,320,240);
    rest(100);
  }
  snap_togrid();
  roll_initiative();
  ...
  while (combatend == 0)
  {
    if (timer_count >= 10)
    {
      ...
      for (z=0;z<PSIZE+numens;z++)
      {
        ...
        {
          if (fighter[z].sts[S_POISON]-1 == rcount && fighter[z].hp > 1)
          {
...
void draw_fighter(int dude,int dcur)
{
  ...
  if (dcur == 1)
    draw_sprite(double_buffer,bptr,xx+(fighter[dude].cw/2)-8,yy-8);
  if (vspell == 1 && dude >= PSIZE)
  {
    ...
    rect(double_buffer,xx-16,yy+fighter[dude].cl+2,xx+15,yy+fighter[dude].cl+5,0);
    if (ff > 20)
      rectfill(double_buffer,xx-15,yy+fighter[dude].cl+3,xx-15+ff-1,yy+fighter[dude].cl+4,40);
    if (ff <= 20 && ff > 10)
      rectfill(double_buffer,xx-15,yy+fighter[dude].cl+3,xx-15+ff-1,yy+fighter[dude].cl+4,104);
    if (ff <= 10 && ff > 0)
      rectfill(double_buffer,xx-15,yy+fighter[dude].cl+3,xx-15+ff-1,yy+fighter[dude].cl+4,24);
  }
}
...
void heroes_win(void)
{
    ...
    if (rand()%100 < fighter[a+PSIZE].dip)
    {
      ...
      if (fitm > 0)
      {
        if (check_inventory(fitm,1) != 0)
        {
          ...
          draw_icon(double_buffer,items[fitm].icon,156-(strlen(strbuf)*4),nr*24+56);
  ...
  for (c=0;c<numchrs;c++)
  {
    if (party[pidx[c]].sts[S_STONE] == 0 && party[pidx[c]].sts[S_DEAD] == 0)
    {
      ...
      sprintf(strbuf,"Next level %7d",party[pidx[c]].next-party[pidx[c]].xp);
