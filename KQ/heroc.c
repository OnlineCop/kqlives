...
char sk_names[MAXCHRS][7] = {"Rage","Sweep","Infuse","Prayer","Boost","Cover","Steal","Sense"};
...
*/
int hero_invokeitem(int whom,int eno)
{
  int tg=0,a,b;
...
void hero_run(int whom)
{
  ...
  if (c == 0)
    program_death("Fatal error: Why are the heroes running from dead enemies?");
  else
    ct = ct / c;
  if (bt > ct)
    a+=25;
...
*/
int combat_castable(int who, int sno)
{
  int b,c=0;
