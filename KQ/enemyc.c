...
int select_encounter(int en, int etid)
{
  ...
  numens = p;
  if (numens > 2 && numchrs == 1 && erows[entry].lvl+2 > party[pidx[0]].lvl && etid == 99)
    numens = 2;
  if (numens == 0)
...
*/
void load_enemyframes(int who,int locx,int locy)
{
  int p;
  DATAFILE *pcx;
  ...
  for (p=0;p<MAXCFRAMES;p++)
  {
    ...
    blit((BITMAP *)pcx->dat,cframes[who][p],locx,locy,0,0,fighter[who].cw,fighter[who].cl);
    blit((BITMAP *)pcx->dat,tcframes[who][p],locx,locy,0,0,fighter[who].cw,fighter[who].cl);
  }
...
void enemy_chooseaction(int who)
{
  ...
  if (fighter[who].hp < fighter[who].mhp*2/3 && rand()%100 < 50 && fighter[who].sts[S_MUTE] == 0)
  {
  ...
  for (a=0;a<8;a++)
  {
    if (ap < fighter[who].aip[a])
    {
      ...
      if (fighter[who].ai[a] >= 1 && fighter[who].ai[a] <= 99 && fighter[who].sts[S_MUTE] == 0)
      {
...
*/
int enemy_cancast(int wh, int sp)
{
  int a,z=0;
...
*/
void enemy_curecheck(int w, int t)
{
  int a;
...
*/
void enemy_spellcheck(int w, int ws)
{
  ...
  if (fighter[w].ai[ws] >= 1 && fighter[w].ai[ws] <= 99)
  {
    ...
    if (cs > 0 && enemy_cancast(w,cs) == 1)
    {
      switch (cs)
      {
        case M_SHIELD: case M_SHIELDALL:
          yes = enemy_stscheck(w,S_SHIELD,PSIZE);
          break;
        case M_HOLYMIGHT:
          aux = 0;
          for (z=PSIZE;z<PSIZE+numens;z++)
            if (fighter[z].sts[S_DEAD] == 0 && fighter[z].sts[S_STRENGTH] < 2)
              aux++;
          if (aux > 0)
            yes = 1;
          break;
        ...
        case M_ABSORB:
          if (fighter[w].hp < fighter[w].mhp/2)
            yes = 1;
          break;
        case M_VENOM: case M_BLIND: case M_CONFUSE: case M_HOLD: case M_STONE: case M_SILENCE: case M_SLEEP:
          yes = enemy_stscheck(w,magic[cs].elem-8,0);
          break;
        case M_NAUSEA: case M_MALISON:
        ...
        case M_DIVINEGUARD:
          aux = 0;
          for (z=PSIZE;z<PSIZE+numens;z++)
            if (fighter[z].sts[S_DEAD] == 0 && fighter[z].sts[S_SHIELD] == 0 && fighter[z].sts[S_RESIST] == 0)
              aux++;
          if (aux > 0)
            yes = 1;
          break;
        case M_DOOM:
          aux = 0;
          for (z=0;z<numchrs;z++)
            if (fighter[z].sts[S_DEAD] == 0 && fighter[z].hp >= fighter[z].mhp/3)
              aux++;
          if (aux > 0)
            yes = 1;
          break;
...
*/
int enemy_stscheck(int who, int ws, int s)
{
  int z,a=0;
...
*/
void enemy_skillcheck(int w, int ws)
{
  int sk;
...
/*
   ...
   by an enemy.
*/
int spell_setup(int whom, int z)
{
  ...
  switch (z)
  {
    ...
    case M_SHELL: case M_WALL:
      zst = S_RESIST;
      break;
    case M_VENOM: case M_HOLD: case M_BLIND: case M_SILENCE: case M_SLEEP: case M_CONFUSE: case M_STONE:
      zst = magic[z].elem-8;
      break;
    case M_NAUSEA:
      zst = S_MALISON;
    ...
    case TGT_ALLY_ONEALL:
      if (z == M_CURE1 || z == M_CURE2 || z == M_CURE3 || z == M_CURE4)
      {
        ...
        for (a=PSIZE;a<PSIZE+numens;a++)
          if (fighter[a].sts[S_DEAD] == 0 && fighter[a].hp < fighter[a].mhp*75/100)
            aux++;
        if (aux > 1)
          fighter[whom].ctmem = SEL_ALL_ALLIES;
        else
...
*/
int skill_setup(int whom, int sn)
{
...
*/
void enemy_attack(int whom)
{
  int a,b,c;

  if (fighter[whom].hp < (fighter[whom].mhp/5) && fighter[whom].sts[S_CHARM] == 0)
  {
    if (rand()%4 == 0)
    {
