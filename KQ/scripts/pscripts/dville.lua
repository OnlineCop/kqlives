...
function entity_handler(en)
  ...
  if (get_progress(55) == 1 and en ~= 7) then
    bubble(en, "You're not welcome here!")
    change_map("main", 256, 77, 256, 77)
    return
  end
  if (en == 0) then
    if (get_progress(55) < 5) then
  ...
  elseif (en == 7) then
    ...
    elseif (get_progress(55) == 2) then
      bubble(en, "Good luck.")
    elseif (get_progress(55) == 3 or
            get_progress(55) == 4) then
      bubble(en, "This is incredible... you actually got the statue back...")
      bubble(200, "...Well, hold on a second. I didn't get the WHOLE statue back; just a part of it. It looks like it had been broken before I got there.")
      ...
      bubble(200, "Don't worry... not everything we adventurers do is for a reward.")
      ...
      if (get_progress(55) == 3) then
        bubble(200, "Well, him being crazy explains why he was rambling on about someone trying to steal the statue from him.")
      else
        bubble(200, "He was rambling on about someone trying to seal the statue from him.")
        if (get_progress(109) == 0) then
          bubble(200, "It still doesn't make any sense.")
          bubble(200, "Wait a minute! I remember that CORIN was down there...")
          if (get_numchrs() == 1) then
            bubble(200, "I might be able to go back there and talk to him.")
          else
            bubble(200, "We might be able to go back there and talk to him.")
          end
          bubble(200, "Maybe he'd know more about it.")
        else
          if (party[0] == 2) then
            CorinInParty = 200
            cp = "I thought"
          elseif (get_numchrs() == 2 and party[1] == 2) then
            CorinInParty = 201
            cp = "I thought"
          else
            CorinInParty = 200
          ...
          if (CorinInParty == 0) then
            if (get_numchrs() == 1) then
              bubble(200, "It didn't make any sense until I met up with CORIN.")
            else
              bubble(200, "It didn't make any sense until we met up with CORIN.")
            end
          end
      ...
      bubble(200, "Well, yeah I guess that's good. And you don't have to worry about any crazed Nerakians for awhile either.")
...
function LOC_trollstory(en)
  ...
  bubble(200, "Have your people tried to get the statue back themselves?")
  ...
  bubble(200, "Are trolls common to this area? I have seen none in my travels.")
  ...
  bubble(200, "Well, don't worry. We will get that statue back for you and get to the bottom of this whole thing.")
