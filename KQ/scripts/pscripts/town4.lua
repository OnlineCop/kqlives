...
function autoexec()
  if (get_progress(39) == 1) then
    if (get_numchrs() > 1) then
      set_progress(40, get_pidx(1) + 1)
      remove_chr(get_pidx(1))
      set_ent_active(201, 0)
    else
      set_progress(40, 0)
    end
    set_progress(39, 2)
  ...
  if (get_progress(41) == 1) then
    set_ent_active(6, 0)
  end
  if (get_ent_tilex(200) > 93 and get_ent_tilex(200) < 107 and
      get_ent_tiley(200) > 0 and get_ent_tiley(200) < 12) then
    set_sstone(0)
...
function postexec()
  ...
  if (get_progress(39) == 2) then
    if (get_progress(40) > 0) then
      bubble(200, "What?")
      set_autoparty(1)
      set_ent_script(200, "F0W25F2W25F1W25F3W25F0")
      wait_for_entity(200, 200)
      set_autoparty(0)
      a = get_progress(40) - 1
      b = "Where's "..get_party_name(a).."?"
      bubble(200, b)
      bubble(200, "Just great... how am I supposed to get back?")
      wait(100)
      if (party[0] == 4) then
        if (get_progress(40) - 1 == 5) then
          bubble(200, "I have to find him! I'm not letting him get away from me that easily.")
        else
          thought(200, "Oh well...")
        end
      else
        bubble(200, "Sigh. I guess I'll just have to keep going and hope I find a way to get back.")
        wait(50)
      end
    end
    set_progress(39, 3)
...
function entity_handler(en)
  if (en == 0) then
    if (get_progress(42) == 0) then
      ...
      if (party[0] == 4) then
        bubble(200, "Oh, not too bad. Needs more flowers, though...")
      end
    end
  ...
  elseif (en == 1) then
    ...
    if (party[0] == 4) then
      bubble(200, "Well, you've certainly done a good job on them. Have you ever considered birchleaf fertilizer for extra color?")
      wait(50)
    end
  ...
  elseif (en == 6) then
    if (get_progress(41) == 0) then
      bubble(en, "Hey, the name's Trezin, and have I got a deal for you!")
      if (party[0] == 4) then
        thought(200, "Oh great. This guy sounds like another used-chariot salesman. Sign...")
      end
      if (prompt(en, 2, 0, "You can have this lovely house",
        ...
        if (get_gp() >= 7500) then
          ...
          if (party[0] == 6) then
            thought(200, "Yea, or swipe it out of your purse, you old windbag...")
          elseif (party[0] == 4) then
            thought(200, "Yea; fear not, maybe I'll even consider buying it!",
                           "Jerk...")
          end
          return
        end
        ...
        if (party[0] == 4) then
          thought(200, "No way! Did he say free?! You mean I can sleep in my OWN house, in my OWN bed for free?! Oh wow!")
        end
        bubble(en, "Secondly, there is a save spot right in the house! How's that for convenience?")
        if (party[0] == 4) then
          thought(200, "Yea, and how's a nice knuckle sandwich sound for convenience? Just LEAVE already you old geezer!")
        end
        ...
        if (party[0] == 4) then
          thought(200, "Not if you're anywhere around it when I get back, it's not!")
        end
        bubble(en, "Well, enough of my yapping. Go in and see for yourself!, I've got to go put this money somewhere safe!")
        if (party[0] == 4) then
          thought(200, "Oh, I can tell you where to put it that'll keep it really safe...")
        end
  ...
  elseif (en == 7) then
    ...
    if (get_progress(50) == 0 and get_progress(43) > 0) then
      if (get_progress(42) == 4) then
        ...
        if (get_progress(49) == 1) then
          bubble(200, "But the Coliseum was closed when I went there.")
          bubble(en, "Yes... well I'm sure it's open by now.")
          bubble(200, "Isn't that convenient?")
          bubble(en, "Quite.")
        end
        bubble(200, "So, I have to go there and ask Trayor for the shield?")
        bubble(en, "No, you have to fight him for it.")
        bubble(200, "Fight, fight, fight. That's all I ever do.")
        bubble(en, "Well, that's just how these things work.")
        bubble(200, "Well, I guess I'm off to fight Trayor. Oh by the way, what kind of battle is this? Is magic permitted?")
        bubble(en, "Yes indeed. Anything goes... and Trayor is a very capable warrior. It won't be easy.")
        bubble(200, "Oh... well nothing is ever easy right?")
        bubble(en, "Not if it's worth doing, no.")
        bubble(200, "Thanks. I'll see you after I get the Shield.")
        set_progress(42, 5)
        set_progress(49, 2)
        return
      else
    ...
    if (get_progress(50) == 1 and get_progress(42) < 6) then
      bubble(en, "Oh... you're back. I'm sorry, but I haven't been able to find out where the other pieces are.")
      bubble(200, "Hey, don't worry about it.")
      bubble(200, "My friend here brought me information about the Band and the Armor. We'll have no trouble finding it now.")
      bubble(200, "I appreciate all your help in finding the Helmet and the Shield. Thanks!")
      bubble(en, "Well, that's great... and you're welcome. I'm glad I was able to help.")
      bubble(200, "I'll see you around then.")
      bubble(en, "Certainly. Good journey.")
      set_progress(42, 6)
      return
    end
    if (get_progress(42) == 6) then
