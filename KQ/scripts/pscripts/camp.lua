...
function zone_handler(zn)
  ...
  elseif (zn == 5) then
    ...
    if (get_numchrs() == 2 and get_ent_tilex(201) == 32 and get_ent_tiley(201) == 49) then
...
function entity_handler(en)
  ...
  elseif (en == 37) then
    ...
    bubble(200, "But how did you end up here?")
    ...
    bubble(200, "Hmm... this almost sounds as though the whole thing was planned.")
    bubble(en, "Hmm...")
    bubble(200, "Excuse me, Mr. Mayor.")
    bubble(2, "Yes?")
    bubble(200, "Do you think that this could have possibly been a planned attack with the purpose of trying to kidnap you?")
    ...
    set_obs(41, 14, 0)
    set_autoparty(1)
    set_ent_script(200, "D2R1F2")
    if (get_numchrs() == 2) then
      if (get_ent_tilex(201) == get_ent_tilex(200) + 1) then
        set_ent_script(201, "L1D3R1F2")
        wait_for_entity(200, 201)
      elseif (get_ent_tilex(201) == get_ent_tilex(200) - 1) then
        set_ent_script(201, "R1D3R1F2")
        wait_for_entity(200, 201)
      else
        set_ent_script(201, "D2R1F2")
        wait_for_entity(200, 201)
      end
    else
    ...
    set_progress(44, 1)
    set_progress(6, 1 + get_pidx(0))
    if (get_numchrs() > 1) then
      set_progress(7, 1 + get_pidx(1))
    end
  end
end
