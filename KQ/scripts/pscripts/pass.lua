...
-- elseif (zn == 1) then
  ...
  elseif (zn == 3) then -- northern door
    if (get_progress(71) == 0) then
      bubble(200, "What's wrong with the code?",
...
function miner(en)
  if (get_progress(87) == 0) then
    bubble(en, "Howdy!")
    bubble(200, "Hello. Is this your cabin?")
    ...
    bubble(200, "Like what?")
    bubble(en, "I guess I've said enough...")
    set_progress(87, 1)
    if (get_progress(71) == 0) then
      bubble(200, "Don't worry. I was just passing through, anyway.")
    end
  elseif (get_progress(87) == 1) then
    bubble(200, "Hello again.")
  ...
  if (get_progress(87)<2) then
    bubble(200, "But, I really need to get in there.")
    set_progress(87, 2)
    bubble(en, "We-e-ell...")
