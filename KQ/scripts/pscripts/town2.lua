...
function autoexec()
  ...
  if (get_progress(5) == 0) then
    while (get_progress(6) == 0) do
      a = krnd(8);
      if (a ~= get_pidx(0)) then
        set_progress(6,a+1);
      end
    end
    while (get_progress(7) == 0) do
      a = krnd(8);
      if (a ~= get_pidx(0) and a ~= get_progress(6)-1) then
        set_progress(7,a+1);
      end
    end
    for a=0,7,1 do
      ...
      if (a == 0 or a == 1 or a == 5 or a == 6) then
        set_all_equip(a,10,38,50,66,74,0);
      elseif (a == 3) then
        set_all_equip(a,2,38,50,56,74,0);
      elseif (a == 2 or a == 4 or a == 7) then
        set_all_equip(a,28,38,45,56,74,0);
      end
    end
    set_progress(5,1);
  end
  if (get_progress(5) == 1) then
    if (get_progress(6) > 0) then
      set_ent_id(8,get_progress(6)-1);
      set_ent_active(8,1);
    end
    if (get_progress(7) > 0) then
      set_ent_id(9,get_progress(7)-1);
      set_ent_active(9,1);
    end
  elseif (get_progress(5) > 1) then
    set_ent_active(8,0);
    set_ent_active(9,0);
  end
  if (get_progress(44) == 1) then
    set_ent_id(10,get_progress(45)-1);
    set_ent_active(10,1);
  else
    set_ent_active(10,0);
  end
...
function zone_handler(zn)
  ...
  elseif (zn == 34) then
    bubble(200,"How to make friends",
                 "through hypnosis.","","");
...
function LOC_partner_check(who)
  if (prompt(who,2,0,"Hi $0, want to team up?","  yes","  no","") == 0) then
    ...
    if (who == 8) then
      add_chr(get_progress(6)-1);
    else
      add_chr(get_progress(7)-1);
    end
    set_progress(5,2);
    give_item(103,3);
    give_item(104,1);
