...
function autoexec()
  ...
  if (get_progress(73) == 1) then
    ...
    set_ent_tilex(6, 57)
    --
    if (get_progress(8) == 1) then
      set_progress(8, 2)
    end
    --
    --
    if (get_progress(44) == 1) then
      set_progress(44, 2)
    end
  end
  refresh()
...
function refresh()
  ...
  --
  if (get_progress(106) == 0) then
    set_ent_active(12, 0)
  end
  --
  if (get_progress(107) == 0) then
    set_ent_active(11, 0)
    set_zone(134, 11, 0)
  end
  if (not LOC_manor_or_party(3)) then
    --
    set_ent_id(10, 3)
  else
    --
...
function zone_handler(zn)
  ...
  elseif (zn == 32) then
    book_talk(party[0])
  elseif (zn == 33) then
    bubble(200, "Hmmm... books about herbs.")
  elseif (zn == 34) then
    if (get_progress(44) > 1) then
...
function entity_handler(en)
  ...
  elseif (en == 3) then
    if (get_progress(44) > 1 and get_progress(106) > 0 and get_progress(107) > 0) then
      bubble(en, "My husband gets so forgetful. Sometimes he doesn't lock up after himself.")
      set_obs(113, 23, 0)
      set_zone(113, 23, 0)
    elseif (get_progress(10) > 4) then
      bubble(en, "Good day.")
    else
      if (get_progress(102) == 0) then
        --
        bubble(en, "I'm just preparing some vegetables.")
        bubble(200, "That's a strange knife you've got there.")
        bubble(en, "What? Oh, this. Yes, it's a Phoenix Blade.")
        bubble(en, "I found it lying about under a pile of leaves in the forest.")
        bubble(200, "Isn't there supposed to be someone out looking for that?")
        ...
        bubble(200, "Your secret's safe with me.")
        set_progress(102, 1)
      else
...
function LOC_join_ajathar(en)
  ...
  if (get_progress(38) == 0) then
    if (get_progress(101) == 0) then
      bubble(200, "Hello! You haven't ventured very far!")
      ...
      bubble(200, "And what if they DO come out?")
      bubble(en, "My training also included swordsmanship.")
      set_progress(101, 1)
    elseif (get_progress(101) == 1) then
      bubble(en, "I hope I am doing the right thing here.")
    end
  else
    if (get_progress(101) == 0) then
      bubble(en, "Be careful! That tunnel is infested with monsters.")
      bubble(200, "Fortunately, I have been successful in closing the Portal. No more monsters will trouble us now.")
      set_progress(101, 1)
    else
      bubble(200, "You can rest easy now. I have closed the Portal that let the monsters through.")
    end
    bubble(en, "Great! Can I offer my services?")
    --
    set_all_equip(3, 2, 38, 50, 56, 74, 0)
    id = select_team{3}
    --
    add_to_manor(id)
    if (id[1]) then
...
function LOC_join_casandra(en)
  ...
  bubble(200, "Don't mention it. How did you get caught up in that mess anyway?")
  ...
  bubble(200, "Hmm, still sounds a bit fishy.")
  bubble(en, "I'd gladly join you to find out what this is all about!")
  --
  set_all_equip(4, 2, 38, 50, 56, 74, 0)
  id = select_team{4}
  --
  add_to_manor(id)
  if (id[1]) then
