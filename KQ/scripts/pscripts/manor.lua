...
function autoexec()
  ...
  if (get_progress(77) == 0) then
    ...
    place_ent(200,get_ent_tilex(get_pidx(0)),get_ent_tiley(get_pidx(0)));
    set_ent_facing(200,get_ent_facing(get_pidx(0)));
    calc_viewport(1);
  end
  if (get_progress(77) > 1) then
    for a=0,7,1 do
      set_ent_active(a,0);
    end
    set_ent_active(8,0);
...
function LOC_talk_butler()  
  ...
  bubble(200,"That kind of thing happens",
  ...
  bubble(9,"You will see them once they",
           ...
           "already too late.","");
  bubble(200,"So, do you mean to say that I",
  ...
  if (get_pidx(0) == 6) then
    bubble(200,"This reward your boss is",
                 "offering better be a lot",
                 "more than just handsome.","");
  else
    bubble(200,"Oh well, I didn't think this",
                 "would be a walk in the park.","","");
  end
  bubble(9,"Yes, well, you had better",
           "be leaving now.","","");
  bubble(200,"Certainly, thanks.","","","");
end
