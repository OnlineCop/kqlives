...
function refresh()
  ...
  if (get_progress(26) == 1) then
    set_ftile(76,9,222);
  elseif (get_progress(26) == 2) then
    set_ftile(76,9,221);
  end
  if (get_progress(27) == 1) then
    set_ftile(80,9,222);
  elseif (get_progress(27) == 2) then
    set_ftile(80,9,221);
  end
  if (get_progress(28) == 1) then
    set_ftile(90,13,222);
  elseif (get_progress(28) == 2) then
    set_ftile(90,13,221);
  end
  if (get_progress(29) == 1) then
    set_ftile(94,13,222);
  elseif (get_progress(29) == 2) then
    set_ftile(94,13,221);
  end
  if (get_progress(20) == 1) then
    set_btile(42,12,190);
    set_btile(47,9,160);
    set_obs(47,9,1);
    set_btile(50,9,0);
  ...
  if (get_progress(302) == 1) then
    set_ftile(83,19,175);
    set_btile(83,20,176);
    set_obs(83,19,0);
    set_zone(83,19,0);
  end
  if (get_progress(34) == 1) then
    set_ftile(78,9,175);
    set_btile(78,10,176);
    set_obs(78,9,0);
    set_zone(78,9,0);
  end
  if (get_progress(51) == 1) then
    set_ftile(89,4,0);
    set_obs(89,4,0);
    set_zone(89,4,0);
  end
  if (get_progress(52) == 1) then
    set_ftile(41,20,0);
    set_obs(41,20,0);
    set_zone(41,20,0);
  end
  if (get_progress(53) == 1) then
    set_ftile(16,13,0);
    set_obs(16,13,0);
    set_zone(16,13,0);
  end
...
function zone_handler(zn)
  ...
  elseif (zn == 11) then
    bubble(200,"Wow! There are a lot",
                 "of strange books here.","","");
  
  elseif (zn == 12) then
    if (get_progress(20) == 0) then
  ...
  elseif (zn == 26) then
    if (get_progress(302) == 0) then
      sfx(26);
      bubble(200,"Oh! I think I stepped",
                   "on a switch!","","");
      set_ftile(83,19,175);
      set_btile(83,20,176);
      set_obs(83,19,0);
  ...
  if (en == 0) then
    if (get_progress(18) == 0) then
      ...
      bubble(0,"They probably have no idea",
               "where to go.","","");
      bubble(200,"Why wouldn't they have spoken to",
      ...
      bubble(0,"However, you should wait until",
               ...
               "of first.");
      bubble(200,"Yes, I suppose there are. Are",
      ...
      set_progress(18,1);
      set_autoparty(1);
      set_ent_script(200,"X150Y10F1");
      if (get_numchrs() == 1) then
        wait_for_entity(200,200);
        bubble(200,"I hope that this Binderak",
                     "guy hasn't gotten a hold",
                     "of the others.","");
        bubble(200,"I don't even want to",
                     "think about that!","","");
        set_ent_facing(200,0);
        drawmap();
        screen_dump();
        bubble(200,"Heh... I should probably",
                     "stop talking to myself too.","","");
      else
        set_ent_script(201,"X150Y11F1");
        wait_for_entity(200,201);
        set_ent_facing(200,0);
        bubble(201,"Hey $0.","","","");
        bubble(200,"Yeah?","","","");
        bubble(201,"Do you think that this Binderak",
                     "could have done something to the",
                     "others?","");
        bubble(200,"You mean capture them?","","","");
        bubble(201,"Or worse. What if he knows about",
                     "our quest and is slowly knocking",
                     "us off?","");
        bubble(200,"I never thought of that. Let's",
                     "hope that he doesn't know about.",
                     "We have no choice but to go",
                     "on anyways.");
        bubble(201,"Yeah, I guess you're right.",
                     "Let's go.","","");
        set_autoparty(0);
        orient_heroes();
      end
...
function LOC_stoner(a,b,c)
  if (get_progress(a) == 0) then
    ...
    if (get_progress(24) == 0 and get_progress(25) > 0) then
      set_progress(a,1);
      set_progress(25,get_progress(25)-1);
      sfx(5);
      set_ftile(b,c,222);
    ...
    if (get_progress(a) ~= 0) then
      if (get_progress(a) == 1 and get_progress(34) == 0) then
        set_progress(a,0);
        set_progress(25,get_progress(25)+1);
        sfx(4);
        set_ftile(b,c,220);
      ...
      if (get_progress(a) == 2 and get_progress(34) == 0) then
        set_progress(a,0);
        set_progress(24,get_progress(24)+1);
        sfx(4);
        set_ftile(b,c,220);
