...
function entity_handler(en)
  ...
  elseif (en == 2) then
    bubble(2, "That statue was given to the village after the evil Kalatos was defeated in the Great War.");

  elseif (en == 3) then
    if (get_progress(P_DENORIAN) < 5) then
      bubble(3, "The village council says that it was a troll who stole the statue. What's a troll?");
    else
  ...
  elseif (en == 4) then
    if (get_progress(P_DENORIAN) < 5) then
      bubble(4, "Apparently, there has not been a troll spotted in these parts since the Great War.");
    else
      bubble(4, "I wonder if our village is really safe?");
    end

  elseif (en == 5) then
    if (get_progress(P_DENORIAN) < 5) then
      bubble(5, "I can't believe this has happened to us. We are a peaceful people.");
    else
  ...
  elseif (en == 6) then
    if (get_progress(P_DENORIAN) < 5) then
      ...
      bubble(6, "Things can get back to normal now.");
    end

  elseif (en == 7) then
    if (get_progress(P_DENORIAN) == 0) then
      bubble(7, "The council would like to ask for your help in retrieving our statue. We will tell you what we know.");
      if (prompt(7, 2, 0, "However, I must first ask if",
                       "you are willing to help us?",
                       "  yes",
                       "  no") == 0) then
        bubble(7, "Oh, thank you very much! We will not forget your kindness.");
        bubble(7, "So, here is what we know. The thief is a troll. Our people have tracked the troll to a cave on the other side of the river.");
        bubble(7, "That is where he can be found.");
        bubble(HERO1, "Have your people tried to get the statue back themselves?");
        bubble(7, "Yes, we have tried, but we quickly learned that we were no match for this enemy.");
        bubble(7, "We could not even overcome the denizens of the cave, never mind the troll himself!");
        ...
        bubble(7, "I see. Well, then you should be on your way then. My people will show you out.");
      ...
      if (get_progress(P_DENORIAN) == 3 or get_progress(P_DENORIAN) == 4) then
        bubble(7, "This is incredible... you actually got the statue back. This is wonderful. I wish we had some way of paying you back.");
        bubble(7, "Unfortunately our village has next to nothing.");
        ...
        bubble(7, "No, I can't say that I have. Well, I guess I'm a little relieved that we don't have a troll problem.");
        bubble(HERO1, "Well, yeah I guess that's good. And you don't have to worry about any crazed Nerakians for awhile either.");
        bubble(7, "That's good to hear. At the very least I can say that you will always be welcome in our village.");
  ...
  elseif (en == 8) then
    if (get_progress(P_DENORIAN) == 0) then
      bubble(8, "This fellow sitting with me is the head of the village council. You should speak with him.");
  ...
  elseif (en == 9) then
    if (get_progress(P_DENORIAN) == 0) then
      bubble(9, "You should hear what the head of the village council has to say.");
    elseif (get_progress(P_DENORIAN) < 5) then
      bubble(9, "Good luck.");
    else
  ...
  elseif (en == 11) then
    if (get_progress(P_DENORIAN) < 5) then
      bubble(11, "This is where the statue was taken from.");
    else
      bubble(11, "The statue will be put back soon.");
    end

  elseif (en == 12) then
    if (get_progress(P_DENORIAN) < 5) then
      bubble(12, "Less council meetings, more cooking!");
    else
  ...
  elseif (en == 13) then
    if (get_progress(P_DENORIAN) < 5) then
      bubble(13, "Geez, I wish I was old enough to go after that smelly old troll! I'd show him a thing or two.");
    else
  ...
  elseif (en == 14) then
    if (get_progress(P_DENORIAN) < 5) then
      bubble(14, "The council is having another meeting about what to do to get back our statue.");
    else
      bubble(14, "I think a celebration is in order.");
    end

  elseif (en == 15) then
    if (get_progress(P_DENORIAN) < 5) then
      bubble(15, "You should find the council's hut. They are looking for people to help get the statue back.");
    else
      bubble(15, "The council can relax now. Thank you.");
    end
