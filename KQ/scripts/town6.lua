  ...
  elseif (zn == 29) then
    if (get_progress(P_SAVEBREANNE) == 1) then
      if (get_numchrs() == 1) then
        ...
        bubble(HERO1, "The same thing happened with the Helmet. The minute I show up to get the damn thing... it's gone.");
        ...
        bubble(HERO2, "We'll be better prepared for Binderak and anybody else who gets in our way.");
...
function entity_handler(en)
  if (en == 0) then
    if (get_progress(P_OPALSHIELD) == 0) then
      bubble(0, "How do you like our little town?");
      return;
    end
    if (get_progress(P_SAVEBREANNE) == 0) then
      bubble(0, "You must help save our daughter! We'll give you anything, just please bring her back to us.");
      if (get_progress(P_TALKGRAMPA) == 1) then
        bubble(HERO1, "We were told she was kidnapped, but by who? Where did they take her?");
        bubble(0, "The ones who took her are members of a cult... the Cult of the Shadow Walker. They took her to their tower on Sharia island.");
      else
        bubble(HERO1, "What happened to her? Where did she go?");
        bubble(0, "She was kidnapped by cult members from the Cult of the Shadow Walker. They took her to their tower on Sharia island.");
      end
      bubble(0, "They have the travel stone to the island guarded by a giant!");
      bubble(HERO1, "Why did they take her?");
      bubble(0, "They took her because she is descended from the legendary half-dragon king Elgar.");
      bubble(0, "They are trying to summon an avatar of the Shadow Walker, and they plan on using Breanne to do it!");
      bubble(HERO1, "Doesn't that mean that one of you is also descended from Elgar?");
      bubble(0, "No. We adopted Breanne eight years ago. She doesn't even know that we are not her real parents!");
      ...
      bubble(0, "Oh, it's on the other side of Kragar's Pass.");
      bubble(0, "I'll send word to the guards to let you through because normally only caravans are allowed into the Pass.");
      ...
      bubble(0, "If you save my daughter, you can have it!");
      bubble(HERO1, "Really? Great! Where is it?");
      bubble(0, "It belongs to my daughter. She is probably wearing it.");
      bubble(HERO1, "Of course. Well, time to go!");
      set_progress(P_SAVEBREANNE, 1);
      return;
    end
    if (get_progress(P_SAVEBREANNE) == 4) then
      bubble(0, "Things are finally back to normal.");
      return;
    end
    if (get_progress(P_SAVEBREANNE) < 3) then
      bubble(0, "Please be careful.");
    else
      bubble(0, "Breanne told us she gave you the Band. We're glad everything has worked out so great. Thank you very much!");
      bubble(0, "I know it's not much, but to show our appreciation, we want you to feel welcome in our town.");
      bubble(0, "Therefore, any time you need to stay at our inn, you'll only be charged 1 gp!");
      set_progress(P_TOWN6INN, 1);
      bubble(HERO1, "That's very kind of you. Thank you!");
      bubble(0, "Oh, and by the way. You should take this.");
      sfx(5);
      ...
      bubble(0, "That will unlock the doors in the mountain pass.");
      bubble(0, "After thinking about this Opal stuff for a while, I remembered a story about the Opal Armor.");
      bubble(0, "Apparently, there is a magical suit of Opal Armor locked away deep in the mountain.");
      bubble(0, "That key will unlock the sealed entrances to the dungeons below.");
      bubble(HERO1, "That's perfect! The next item we were on our way to find was the Opal Armor.");
      bubble(0, "Well, I'm glad we could help. However, I must warn you to be extra cautious on this quest.");
      bubble(0, "A dark race of Larinon live in the dungeons down there and they are most unpleasant.");
  ...
  elseif (en == 2) then
    bubble(2, "This is a lovely little town. Don't you think so?");
  ...
  elseif (en == 4) then
    bubble(4, "Sorry, I'm far too busy and you have terrible breath.");

  elseif (en == 5) then
    if (get_progress(P_SAVEBREANNE) < 3) then
      bubble(5, "There has been a lot of Cult activity around these parts lately.");
    else
      bubble(5, "Has the Cult really been rousted?");
    end

  elseif (en == 6) then
    bubble(6, "Lord and Lady Ajantara have hired us to watch the town.");

  elseif (en == 7) then
    if (get_progress(P_OPALSHIELD) == 0) then
      bubble(7, "Lord Ajantara's grandfather founded this town.");
      return;
    end
    if (get_progress(P_SAVEBREANNE) == 0) then
      if (get_progress(P_TALKGRAMPA) == 0) then
        bubble(7, "Lord and Lady Ajantara's daughter, Breanne, was kidnapped last night. They are looking for someone to help find her!");
        set_progress(P_TALKGRAMPA, 1);
      else
        bubble(7, "You should go and speak with the Ajantaras.");
      end
    else
      if (get_progress(P_SAVEBREANNE) < 3) then
        bubble(7, "You truly are kind.");
      else
        bubble(7, "It's a lovely day isn't it?");
      end
    end

  elseif (en == 8) then
    bubble(8, "Everything in this town is owned by Lord and Lady Ajantara.");
  ...
  elseif (en == 9) then
    ...
    bubble(9, "Thank you very much. Please feel free to come and visit any time.");
