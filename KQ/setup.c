...
char keynames[115][12] = {
  ...
  "4_PAD","5_PAD","6_PAD","7_PAD","8_PAD","9_PAD","F1","F2","F3","F4",
  "F5","F6","F7","F8","F9","F10","F11","F12","ESC","TILDE",
  "MINUS","EQUALS","BACKSPACE","TAB","OPENBRACE","CLOSEBRACE","ENTER","COLON","QUOTE","BACKSLASH",
  "BACKSLASH2","COMMA","STOP","SLASH","SPACE","INSERT","DEL","HOME","END","PGUP",
  "PGDN","LEFT","RIGHT","UP","DOWN","SLASH_PAD","ASTERISK","MINUS_PAD","PLUS_PAD","DEL_PAD",
  "ENTER_PAD","PRTSCR","PAUSE","ABNT_C1","YEN","KANA","CONVERT","NOCONVERT","AT","CIRCUMFLEX",
  "COLON2","KANJI","LSHIFT","RSHIFT","LCONTROL","RCONTROL","ALT","ALTGR","LWIN","RWIN",
...
void config_menu(void)
{
  ...
  while (!stop)
  {
    if (rd)
    {
      ...
      print_font(double_buffer,96+xofs,8+yofs,"KQ Configuration",FGOLD);
      menubox(double_buffer,32+xofs,24+yofs,30,17,BLUE);
      print_font(double_buffer,48+xofs,32+yofs,"Windowed Mode:",FNORMAL);
      print_font(double_buffer,48+xofs,40+yofs,"Stretch Display:",FNORMAL);
      print_font(double_buffer,48+xofs,48+yofs,"Show Frame Rate:",FNORMAL);
      print_font(double_buffer,48+xofs,56+yofs,"Wait for Retrace:",FNORMAL);
      print_font(double_buffer,48+xofs,72+yofs,"Up Key:",FNORMAL);
      print_font(double_buffer,48+xofs,80+yofs,"Down Key:",FNORMAL);
      print_font(double_buffer,48+xofs,88+yofs,"Left Key:",FNORMAL);
      print_font(double_buffer,48+xofs,96+yofs,"Right Key:",FNORMAL);
      print_font(double_buffer,48+xofs,104+yofs,"Confirm Key:",FNORMAL);
      print_font(double_buffer,48+xofs,112+yofs,"Cancel Key:",FNORMAL);
      print_font(double_buffer,48+xofs,120+yofs,"Menu Key:",FNORMAL);
      print_font(double_buffer,48+xofs,128+yofs,"System Menu Key:",FNORMAL);
      print_font(double_buffer,48+xofs,144+yofs,"Sound System:",FNORMAL);
      if (is_sound == 2)
      {
        print_font(double_buffer,48+xofs,152+yofs,"Sound Volume:",FNORMAL);
        print_font(double_buffer,48+xofs,160+yofs,"Music Volume:",FNORMAL);
      }
      if (is_sound == 0)
      {
        print_font(double_buffer,48+xofs,152+yofs,"Sound Volume:",FDARK);
        print_font(double_buffer,48+xofs,160+yofs,"Music Volume:",FDARK);
      }
      if (windowed == 1)
        print_font(double_buffer,256+xofs,32+yofs,"YES",FNORMAL);
      else
        print_font(double_buffer,264+xofs,32+yofs,"NO",FNORMAL);
      if (stretch_view == 1)
        print_font(double_buffer,256+xofs,40+yofs,"YES",FNORMAL);
      else
        print_font(double_buffer,264+xofs,40+yofs,"NO",FNORMAL);
      if (show_frate == 1)
        print_font(double_buffer,256+xofs,48+yofs,"YES",FNORMAL);
      else
        print_font(double_buffer,264+xofs,48+yofs,"NO",FNORMAL);
      if (wait_retrace == 1)
        print_font(double_buffer,256+xofs,56+yofs,"YES",FNORMAL);
      else
        print_font(double_buffer,264+xofs,56+yofs,"NO",FNORMAL);
      print_font(double_buffer,280-(strlen(keynames[kup])*8)+xofs,72+yofs,keynames[kup],FNORMAL);
      print_font(double_buffer,280-(strlen(keynames[kdown])*8)+xofs,80+yofs,keynames[kdown],FNORMAL);
      print_font(double_buffer,280-(strlen(keynames[kleft])*8)+xofs,88+yofs,keynames[kleft],FNORMAL);
      print_font(double_buffer,280-(strlen(keynames[kright])*8)+xofs,96+yofs,keynames[kright],FNORMAL);
      print_font(double_buffer,280-(strlen(keynames[kalt])*8)+xofs,104+yofs,keynames[kalt],FNORMAL);
      print_font(double_buffer,280-(strlen(keynames[kctrl])*8)+xofs,112+yofs,keynames[kctrl],FNORMAL);
      print_font(double_buffer,280-(strlen(keynames[kenter])*8)+xofs,120+yofs,keynames[kenter],FNORMAL);
      print_font(double_buffer,280-(strlen(keynames[kesc])*8)+xofs,128+yofs,keynames[kesc],FNORMAL);
      if (is_sound == 0)
      {
        print_font(double_buffer,256+xofs,144+yofs,"OFF",FNORMAL);
        print_font(double_buffer,256+xofs,152+yofs,"N/A",FDARK);
        print_font(double_buffer,256+xofs,160+yofs,"N/A",FDARK);
      }
      else
      {
        print_font(double_buffer,264+xofs,144+yofs,"ON",FNORMAL);
        sprintf(strbuf,"%3d%%",gsvol*100/250);
        print_font(double_buffer,248+xofs,152+yofs,strbuf,FNORMAL);
        sprintf(strbuf,"%3d%%",gmvol*100/250);
        print_font(double_buffer,248+xofs,160+yofs,strbuf,FNORMAL);
      }
      p = ptr;
      if (ptr > 3)
        p++;
...
int getavalue(char *capt, int minu, int maxu, int cv, int sp)
{
  int stop=0,a,b,rd=1;
...
void load_samples(void)
{
  ...
  {
    if (!(sfx[i] = load_datafile_object(SOUND_DATAFILE,sndfiles[i])))
    {
