...
*/
int need_spell(int ca, int ta, int sn)
{
  int a,b,vc=0,cs;
  ...
  switch (sn)
  {
    ...
    case M_RECOVERY:
      ...
      if (a == 0 || party[vc].sts[S_DEAD] != 0)
        return 0;
      break;
    case M_LIFE: case M_FULLLIFE:
      if (party[vc].sts[S_DEAD] == 0)
        return 0;
      break;
    case M_CURE1: case M_CURE2: case M_CURE3: case M_CURE4:
...
*/
void camp_draw_spell_menu(int c, int pg, int ptr)
{
  int l,z,j,b,a;
...
*/
void camp_spell_targeting(int mc, int sn)
{
  int tg=0,a;
  ...
  while (tg > -1)
  {
    ...
    {
      tg = select_any_player(magic[sn].tgt-1,magic[sn].icon,magic[sn].name);
      if (tg < 0)
        return;
...
*/
int camp_castable(int who, int sno)
{
