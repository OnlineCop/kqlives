...
void show_sgstats(int saving)
{
  ...
  for (sg=0;sg<NUMSG;sg++)
  {
    ...
    if (snc[sg] == -1)
      print_font(double_buffer,136,sg*48+20,"Wrong version",FNORMAL);
    else
    {
      if (snc[sg] == 0)
...
int confirm_save(void)
{
  ...
  print_font(double_buffer,136,save_ptr*48+20,"Confirm/Cancel",FNORMAL);
  blit2screen(0,0);
...
int start_menu(int c)
{
  ...
  if (debugging == 0)
  {
    ...
    for (a=0;a<16;a++)
    {
      ...
      masked_blit((BITMAP *)bg->dat,double_buffer,0,148,0,172,320,52);
      blit2screen(0,0);
      if (a == 0)
        rest(500);
      else
