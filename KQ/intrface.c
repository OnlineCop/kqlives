...
{
  set_btile((int)lua_tonumber(L,1),(int)lua_tonumber(L,2),(int)lua_tonumber(L,3));
  return 0;
}

int KQ_set_mtile(lua_State *L)
{
  set_mtile((int)lua_tonumber(L,1),(int)lua_tonumber(L,2),(int)lua_tonumber(L,3));
  return 0;
}

int KQ_set_ftile(lua_State *L)
{
  set_ftile((int)lua_tonumber(L,1),(int)lua_tonumber(L,2),(int)lua_tonumber(L,3));
  return 0;
}

int KQ_set_obs(lua_State *L)
{
  set_obs((int)lua_tonumber(L,1),(int)lua_tonumber(L,2),(int)lua_tonumber(L,3));
  return 0;
}

int KQ_set_zone(lua_State *L)
{
  set_zone((int)lua_tonumber(L,1),(int)lua_tonumber(L,2),(int)lua_tonumber(L,3));
  return 0;
}

int KQ_set_shadow(lua_State *L)
{
  set_shadow((int)lua_tonumber(L,1),(int)lua_tonumber(L,2),(int)lua_tonumber(L,3));
  return 0;
}
...
int KQ_create_df(lua_State *L)
{
  g_df = load_datafile_object(lua_tostring(L,1),lua_tostring(L,2));
  return 0;
}

int KQ_destroy_df(lua_State *L)
{
  unload_datafile_object(g_df);
  return 0;
}

int KQ_create_bmp(lua_State *L)
{
  int a = lua_tonumber(L,1);
...
int KQ_destroy_bmp(lua_State *L)
{
  int a = lua_tonumber(L,1);
...
int KQ_df2bmp(lua_State *L)
{
  int a = lua_tonumber(L,1);

  if (a >= 0 && a <= 4)
    blit((BITMAP *)g_df->dat,g_bmp[a],lua_tonumber(L,2),lua_tonumber(L,3),lua_tonumber(L,4),lua_tonumber(L,5),lua_tonumber(L,6),lua_tonumber(L,7));
  return 0;
}

int KQ_blit(lua_State *L)
{
  int a = lua_tonumber(L,1);

  if (a >= 0 && a <= 4)
    blit(g_bmp[a],double_buffer,lua_tonumber(L,2),lua_tonumber(L,3),lua_tonumber(L,4)+xofs,lua_tonumber(L,5)+yofs,lua_tonumber(L,6),lua_tonumber(L,7));
  return 0;
}

int KQ_maskblit(lua_State *L)
{
  int a = lua_tonumber(L,1);

  if (a >= 0 && a <= 4)
    masked_blit(g_bmp[a],double_buffer,lua_tonumber(L,2),lua_tonumber(L,3),lua_tonumber(L,4)+xofs,lua_tonumber(L,5)+yofs,lua_tonumber(L,6),lua_tonumber(L,7));
  return 0;
}
...
int KQ_drawsprite(lua_State *L)
{
  ...
  if (a >= 0 && a <= 4)
    draw_sprite(double_buffer,g_bmp[a],lua_tonumber(L,2)+xofs,lua_tonumber(L,3)+yofs);
  return 0;
}
...
int KQ_prompt(lua_State *L)
{
  ...
  lua_pushnumber(L,prompt(b,lua_tonumber(L,2),lua_tonumber(L,3),txt[0],txt[1],txt[2],txt[3]));
  return 1;
}
    ...
    case 6:
      switch (rand()%3)
      {
        case 0:
          ...
          break;
        case 1:
          bubble_text(0,"What language is this",
                        "written in?","","");
          break;
        case 2:
...
int KQ_door_in(lua_State *L)
{
  ...
  blit2screen(xofs,yofs);
  rest(50);
  set_view(1,lua_tonumber(L,3),lua_tonumber(L,4),lua_tonumber(L,5),lua_tonumber(L,6));
...
{
  message((char *)lua_tostring(L,1),lua_tonumber(L,2),lua_tonumber(L,3),xofs,yofs);
  return 0;
}
...
{
  menubox(double_buffer,lua_tonumber(L,1)+xofs,lua_tonumber(L,2)+yofs,lua_tonumber(L,3),lua_tonumber(L,4),BLUE);
  return 0;
}

int KQ_dark_mbox(lua_State *L)
{
  menubox(double_buffer,lua_tonumber(L,1)+xofs,lua_tonumber(L,2)+yofs,lua_tonumber(L,3),lua_tonumber(L,4),DARKBLUE);
  return 0;
}

int KQ_light_mbox(lua_State *L)
{
  menubox(double_buffer,lua_tonumber(L,1)+xofs,lua_tonumber(L,2)+yofs,lua_tonumber(L,3),lua_tonumber(L,4),DARKRED);
  return 0;
}
...
int KQ_drawframe(lua_State *L)
{
  ...
  draw_sprite(double_buffer,frames[a][b],lua_tonumber(L,3)+xofs,lua_tonumber(L,4)+yofs);
  return 0;
}
...
int KQ_draw_pstat(lua_State *L)
{
  ...
  if (a >= 0 && a <= 7)
    draw_playerstat(double_buffer,a,lua_tonumber(L,2)+xofs,lua_tonumber(L,3)+yofs);
  return 0;
}

int KQ_ptext(lua_State *L)
{
  print_font(double_buffer,lua_tonumber(L,1)+xofs,lua_tonumber(L,2)+yofs,(char *)lua_tostring(L,3),lua_tonumber(L,4));
  return 0;
}
...
int KQ_pnum(lua_State *L)
{
  ...
  sprintf(strbuf,"%d",a);
  print_font(double_buffer,lua_tonumber(L,1)+xofs,lua_tonumber(L,2)+yofs,strbuf,lua_tonumber(L,4));
  return 0;
}
