...
*/
void special_spells(int cs, int sn)
{
  if (cs >= PSIZE)
  {
    ...
    case M_WARP:
      ...
      else
      {
        if (g_map.map_no == MAP_MAIN)
          change_map("town4",g_map.warpx,g_map.warpy,g_map.warpx,g_map.warpy);
        else
...
*/
void cure_oneall_allies(int cs, int tgt, int sn)
{
  ...
  spwr = magic[sn].dmg + (fighter[cs].stats[A_INT+magic[sn].stat]*magic[sn].bon/100);
...
*/
void heal_one_ally(int cs, int tgt, int sn)
{
  int a,b=0;
...
*/
void geffect_one_ally(int cs, int tgt, int sn)
{
  if (rand()%100+1 > fighter[cs].stats[A_AUR+magic[sn].stat] || fighter[tgt].sts[S_STONE] > 0)
  {
    ta[tgt] = MISS;
    return;
  }
...
*/
void geffect_all_allies(int cs, int sn)
{
  int nt,st,a,b=0;
...
*/
void beffect_one_enemy(int cs, int tgt, int sn)
{
  int r,a=0,sp_hit;
...
*/
void beffect_all_enemies(int cs, int sn)
{
  int nt,st,a,sp_hit;
  ...
  switch(sn)
  {
    case M_SLOW:
      for (a=st;a<st+nt;a++)
      {
        if (res_throw(a,magic[sn].elem) == 0 && non_dmg_save(a,sp_hit) == 0 && fighter[a].sts[S_STONE] == 0)
        {
    ...
    case M_MALISON:
      for (a=st;a<st+nt;a++)
      {
        if (non_dmg_save(a,sp_hit) == 0 && fighter[a].sts[S_MALISON] == 0 && fighter[a].sts[S_STONE] == 0)
        {
    ...
    case M_SLEEPALL:
      for (a=st;a<st+nt;a++)
      {
        if (res_throw(a,magic[sn].elem) == 0 && non_dmg_save(a,sp_hit) == 0 && fighter[a].sts[S_SLEEP] == 0 && fighter[a].sts[S_STONE] == 0)
        {
...
*/
void damage_all_enemies(int cs, int sn)
{
  int nt,st;
...
void damage_oneall_enemies(int cs, int tgt, int sn)
{
  int nt,st;
...
*/
void special_damage_oneall_enemies(int cs, int sp_dmg, int rt, int tgt, int split)
{
  ...
  for (a=st;a<st+nt;a++)
  {
    if (fighter[a].sts[S_DEAD] == 0 && fighter[a].mhp > 0)
    {
      ...
      if (b < 0)
        b = 0;
      b = res_adjust(a,rt,b);
      if (fighter[a].sts[S_STONE] > 0 && rt != R_BLACK && rt != R_WHITE && rt != R_EARTH && rt != R_WATER)
        b = b / 10;
      ta[a] = 0-b;
      if (b < 0 && rt == R_POISON)
      {
...
*/
void spell_damage(int cs, int sn, int st, int nt)
{
  ...
  if (ne == 0)
    return;
  rt = magic[sn].elem;
  ad = magic[sn].dmg + (fighter[cs].stats[A_INT+magic[sn].stat]*magic[sn].bon/100);
  if (ad < DMG_RND_MIN*5)
    ad += rand()%DMG_RND_MIN;
  else
    ad += rand()%(ad/5);
...
*/
void set_timed_sts_effect(int who, int ss)
{
