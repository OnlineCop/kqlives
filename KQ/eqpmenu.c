...
*/
void draw_equipmenu(int c, int sel)
{
  int l,j,k;
  ...
  print_font(double_buffer,268-(strlen(party[l].name)*4)+xofs,76+yofs,party[l].name,FNORMAL);
  ...
  for (k=0;k<6;k++)
  {
    ...
    print_font(double_buffer,92+xofs,k*8+36+yofs,items[j].name,FNORMAL);
  }
}
...
*/
void draw_equippable(int c, int slot, int pptr)
{
  int z,j,k;
  ...
  for (k=0;k<sm;k++)
  {
    ...
    print_font(double_buffer,36+xofs,k*8+100+yofs,items[j].name,FNORMAL);
    if (z > 1)
    {
      sprintf(strbuf,"^%d",z);
...
void calc_possible_equip(int c, int slot)
{
  ...
  for (k=0;k<MAX_INV;k++)
  {
    ...
    {
      if (items[g_inv[k][0]].type == slot && items[g_inv[k][0]].eq[pidx[c]] != 0)
      {
        t_inv[tot] = k;
        tot++;
      }
...
*/
void calc_equippreview(int aa,int p2,int ii)
{
  int c,z;
...
void draw_equippreview(int ch, int ptr, int pp)
{
  ...
  if (ptr >= 0)
  {
    ...
    if (c1 < c2)
      print_font(double_buffer,212+xofs,220+yofs,"Resist up",FNORMAL);
    if (c1 > c2)
      print_font(double_buffer,204+xofs,220+yofs,"Resist down",FNORMAL);
  }
}
...
/*
   ...
   buy them.
*/
int equip(int c, int itm, int forced)
{
  int a,d,b,z,n=0,i;
