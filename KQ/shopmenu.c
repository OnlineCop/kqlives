...
void draw_sideshot(int itm)
{
  ...
  for (a=0;a<numchrs;a++)
  {
    ...
    if (slot < 6)
    {
      ...
      {
        for (j=0;j<13;j++)
          cs[j] = items[itm].stats[j] - items[party[pidx[a]].eqp[slot]].stats[j];
      }
      else
      {
        for (j=0;j<13;j++)
...
int shop(int shop_num)
{
  ...
  for (a=0;a<12;a++)
  {
    if (shops[shop_no][a][2] > 0)
      if ((khr*60)+kmin-progress[P_SHOPSTART+shop_no] > shops[shop_no][a][2])
    ...
    if (rd == 1)
    {
      ...
      for (i=0;i<noi;i++)
      {
        ...
        if (max > 0)
        {
          ...
          print_font(double_buffer,200+xofs,i*8+32+yofs,"Sold Out!",k);
      }
      print_font(double_buffer,160-(strlen(items[shin[yptr]].desc)*4)+xofs,176+yofs,items[shin[yptr]].desc,FNORMAL);
...
*/   
void buy_item(int how_many, int item_no)
{
  ...
  while (!stop)
  {
    if (rd == 1)
    {
      ...
      print_font(double_buffer,104+xofs,176+yofs,"Confirm/Cancel",FNORMAL);
...
void sell_menu(void)
{
  ...
  while (!stop)
  {
    if (rd == 1)
    {
      ...
      else
      {
        if (g_inv[pg*16+yptr][0] > 0)
          print_font(double_buffer,76+xofs,192+yofs,"That can not be sold!",FNORMAL);
      }
      draw_sprite(double_buffer,menuptr,32+xofs,yptr*8+32+yofs);
    ...
    {
      unpress();
      if (g_inv[pg*16+yptr][0] > 0 && items[g_inv[pg*16+yptr][0]].price > 0)
...
*/
void sell_howmany(int itm_no, int pg)
{
  ...
  while (!stop)
  {
    if (rd == 1)
    {
      ...
      menubox(double_buffer,32+xofs,itm_no*8+24+yofs,30,1,DARKBLUE);
      draw_icon(double_buffer,items[l].icon,48+xofs,itm_no*8+32+yofs);
      print_font(double_buffer,56+xofs,itm_no*8+32+yofs,items[l].name,FNORMAL);
      sprintf(strbuf,"%d of %d",my,maxi);
      print_font(double_buffer,280-(strlen(strbuf)*8)+xofs,itm_no*8+32+yofs,strbuf,FNORMAL);
      blit2screen(xofs,yofs);
    }
    readcontrols();
    if (up && my < maxi)
...
*/
void sell_item(int itno, int ni)
{
  int l,stop=0,sp,a;
  ...
  print_font(double_buffer,104+xofs,200+yofs,"Confirm/Cancel",FNORMAL);
  blit2screen(xofs,yofs);
  while (!stop)
  {
    readcontrols();
...
void inn(char *iname, int gpc, int pay)
{
  ...
  if (gp >= gpts)
  {
    ...
    print_font(double_buffer,60+xofs,108+yofs,"Do you wish to stay?",FNORMAL);
  }
  else
  {
    menubox(double_buffer,32+xofs,96+yofs,30,2,BLUE);
    print_font(double_buffer,40+xofs,108+yofs,"You can't afford to stay here.",FNORMAL);
    blit2screen(xofs,yofs);
    wait_enter();
    return;
  }
